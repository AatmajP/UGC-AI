        ...
        >
            <div className='max-w-6xl mx-auto flex items-center justify-between bg-black/50 backdrop-blur-md border border-white/4 rounded-2xl p-3'>
                <Link to='/' onClick={() => scrollTo(0, 0)} className="flex items-center gap-2">
                    ...
                    <span className="text-xl font-bold tracking-tighter text-white">UGC.AI</span>
                </Link>
            ...
            <div className={`flex flex-col items-center justify-center gap-6 text-lg font-medium fixed inset-0 bg-black/40 backdrop-blur-md z-50 transition-all duration-300 ${isOpen ? "translate-x-0" : "translate-x-full"}`}>
                {navLinks.map((link) => (
                    <Link key={link.name} to={link.href} onClick={() => setIsOpen(false)} className="text-white">
                        {link.name}
                    </Link>
                ))}
                ...
                {!user ? (
                    <>
                        <button onClick={() => { openSignIn(); setIsOpen(false); }} className='font-medium text-gray-300 hover:text-white transition'>
                            Sign in
                        </button>
                        <PrimaryButton onClick={() => { openSignUp(); setIsOpen(false); }}>Get Started</PrimaryButton>
                    </>
                ) : (
